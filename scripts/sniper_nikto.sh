...
echo -e "$OKGREEN + -- ----------------------------=[Running NIKTO Web Vuln Scan]=---- -- +$RESET"
nikto -h http://$TARGET:$MODE -output "$LOOT_DIR"/nmap/nmapNIKTO80-"$TARGET"_"$MODE"_"$(date +'%FT%H%M%S')".xml
echo -e "$OKGREEN + -- ----------------------------=[Ending NIKTO Web Vuln Scan]=---- -- +$RESET"
exit
